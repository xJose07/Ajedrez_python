...
class Rey(Figura):
    ...
    def movimientoFiguraValido(self, filaInicial, columnaInicial, filaFinal, columnaFinal, figuraEnemiga, figuraAmiga):
        ...
        if ((coordFinal[1] == coordInicial[1] + 1) or (coordFinal[1] == coordInicial[1] - 1)) and not figuraAmiga:    #Si se mueve la columna en 1 o -1
            if (coordFinal[0] == coordInicial[0]):
                movimientoValido = True

        if ((coordFinal[1] == coordInicial[1] + 2) or (coordFinal[1] == coordInicial[1] - 2)) and (coordFinal[0] == coordInicial[0]) and self.primerMovimiento == True and not figuraAmiga:   #Para el enroque necesitamos movernos dos posiciones en la misma fila
            movimientoValido = True                                             
